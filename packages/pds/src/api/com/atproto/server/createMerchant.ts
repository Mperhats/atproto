...
import { InputSchema as CreateMerchantInput } from '../../../../lexicon/types/com/atproto/server/createMerchant'
import AppContext from '../../../../context'
import { didDocForSession } from './util'

export default function (server: Server, ctx: AppContext) {
  server.com.atproto.server.createMerchant({
    rateLimit: {
      durationMs: 5 * MINUTE,
      points: 100,
    },
    ...
    handler: async ({ input, auth, req }) => {
      ...
      try {
        ...
        if (!deactivated) {
          ...
          await ctx.sequencer.sequenceIdentityEvt({did,actorCategory:'merchant'})
        }
